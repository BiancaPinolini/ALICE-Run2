...
//#include "AliPPVsMultUtils.h" // for Run 1
...
void AliAnalysisTaskNuclei::UserCreateOutputObjects()
{
  ...
  AliESDtrackCuts* esdTrackCuts = AliESDtrackCuts::GetStandardITSTPCTrackCuts2011(kFALSE, 1);//0 was for 2010
  ...
  hrapidity[0] = new TH1F("hrapidity_0","Rapidity (assuming deuteron mass) before |y|<0.5 cut;rapidity",1000,-2,2);
  hrapidity[1] = new TH1F("hrapidity_1","Rapidity (assuming deuteron mass) after |y|<0.5 cut;rapidity",1000,-2,2);
  if(isMC) {
    hrapidity[0]->SetTitle("Rapidity before |y|<0.5 cut");
    hrapidity[1]->SetTitle("Rapidity after |y|<0.5 cut");
  }

  hCheckTrackSel = new TH1F("hCheckTrackSel","Number of tracks per event after the track selection",13,0,13);
  const Char_t *xaxisTitle3[13]={"|y|<0.5", "nCrossedRowsTPC>=70", "RatioCROverNclsFTPC>=0.8" ,"chi2perTPCcluster<=4","isTPCrefit","isITSrefit","nSPD>0","NoKinkDaughters","chi2perITScluster<=36","isPropagatedToDca",Form("|DCAxy|<%.1f",DCAxyMax),Form("|DCAz|<%.1f",DCAzMax),"|#eta|<0.8"};
  ...
  hchi2TPC[0] = new TH1F("hchi2TPC_0","#chi^{2} per TPC cluster (before track cuts)",100,0,6);
  ...
  hsigmaToVtx[0] = new TH1F("hsigmaToVtx_0","Number of sigma to the vertex (before track cuts)",100,0,10);
  hchi2ITS[0] = new TH1F("hchi2ITS_0","#chi^{2} per ITS cluster (before track cuts)",100,0,40);
  ...
  hsigmaToVtx[1] = new TH1F("hsigmaToVtx_1","Number of sigma to the vertex (after track cuts)",100,0,10);
  hchi2ITS[1] = new TH1F("hchi2ITS_1","#chi^{2} per ITS cluster (after track cuts)",100,0,40);
...
//**************************************************************** N SIGMA TPC
  ...
  if(isMC) {
	...
	for(Int_t i=0;i<2;i++) fList->Add(htriggerMask[i]);
  fList->Add(hzvertex);
  fList->Add(hNevent);
  ...
  for(Int_t i=0;i<2;i++) fList->Add(hnCrossedRowsTPC[i]);
  for(Int_t i=0;i<2;i++) fList->Add(hRatioCrossedRowsOverFindableClustersTPC[i]);
  ...
  for(Int_t i=0;i<18;i++) fList->Add(fDcaxy[0][i]);
  ...
  for(Int_t i=0;i<18;i++) fList->Add(fDcaz[i]);
	...
	for(Int_t i=0;i<18;i++) fList->Add(fNsigmaTOF[i]);
	for(Int_t i=0;i<2;i++) fList->Add(fBetaTOFvspt[i]);
	for(Int_t i=0;i<9;i++) fList->Add(hBetaExp[i]);

	for(Int_t i=0;i<18;i++) fList->Add(fSparseM2vspt[i]);

	for(Int_t j=0;j<2;j++) {
		for(Int_t i=0;i<2;i++) fList->Add(fM2tof[j][i]);
	}
	for(Int_t j=0;j<2;j++) {
		for(Int_t i=0;i<18;i++) fList->Add(fM2vspt[j][i]);
	}
  ...
  if(isMC) {
    ...
    fList->Add(hMCTrackletsVsV0mult[0]);
		...
		for(Int_t i=0;i<18;i++) fList->Add(fSparsehpt[i]);

    fList->Add(htest[0]);

		for(Int_t k=0;k<4;k++) {
			for(Int_t j=0;j<3;j++) {
				for(Int_t i=0;i<18;i++) fList->Add(hpt[k][j][i]);
			}
		}

		for(Int_t j=0;j<2;j++) {
			for(Int_t i=0;i<18;i++) fList->Add(fptRecoVsTrue[j][i]);
		}

		for(Int_t k=0;k<3;k++) {
			for(Int_t j=0;j<2;j++) {
				for(Int_t i=0;i<18;i++) {
					fList->Add(fmcDca[k][j][i]);
					fList->Add(fmcNsigmaTOF[k][j][i]);
				}
			}
		}
...
void AliAnalysisTaskNuclei::UserExec(Option_t *)
{
  ...
  if(isMC) {
    ...
    Float_t Ntracklets_bes = -1;//(Float_t) fPPVsMultUtils->GetStandardReferenceMultiplicity(fEvent, kFALSE);
    if(Ntracklets_bes<0) Ntracklets_bes=-1;

    Float_t mult_bes = MultSelection->GetMultiplicityPercentile("V0M");
  ...
  ///if(!fPPVsMultUtils->IsEventSelected(fEvent, AliVEvent::kINT7)) return;

  if (!fEventCuts.AcceptEvent(fEvent)) return;

  AliMultSelection *MultSelection = (AliMultSelection *) fEvent->FindListObject("MultSelection");
  Float_t mult = MultSelection->GetMultiplicityPercentile("V0M");

  if(!this->IsInsideFullMultRange(mult)) return;
  hV0mult->Fill(mult);
  //------------------------- Event selection (end)

  Float_t Ntracklets = (Float_t) -1;//fPPVsMultUtils->GetStandardReferenceMultiplicity(fEvent, kFALSE);//kFALSE because I made the event selection before
...
void AliAnalysisTaskNuclei::EventSelectionMonitor() {
  /*
  if(fPPVsMultUtils->IsEventSelected(fEvent, AliVEvent::kINT7)) {
    hNevent->Fill(5);
  }

  if(fPPVsMultUtils->IsSelectedTrigger(fEvent, AliVEvent::kINT7)) {
    hNevent->Fill(0);
...
//_____________________________________________________________________________
Bool_t AliAnalysisTaskNuclei::IsInsideFullMultRange(Float_t multiplicity) {
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskNuclei::GetMultiplicityBin(Float_t multiplicity) {
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskNuclei::GetTrackletsBin(Int_t Ntracklets) {
...
//_____________________________________________________________________________
Double_t AliAnalysisTaskNuclei::GetRapidity(AliVTrack *track) {
...
//_____________________________________________________________________________
Bool_t AliAnalysisTaskNuclei::AcceptTrack(AliVTrack *track, Double_t &DCAxy, Double_t &DCAz) {
  ...
  if(nITSclusters!=0) chi2ITS=track->GetITSchi2()/Double_t(nITSclusters);
	...
	Float_t nTPCNclsF = track->GetTPCNclsF();
	Float_t RatioCrossedRowsOverFindableClustersTPC = nCrossedRowsTPC / nTPCNclsF;
  ...
  hisPropagatedToDca[0]->Fill(isPropagatedToDca);
	hnCrossedRowsTPC[0]->Fill(nCrossedRowsTPC);
	hRatioCrossedRowsOverFindableClustersTPC[0]->Fill(RatioCrossedRowsOverFindableClustersTPC);

  this->TrackSelectionMonitor(nCrossedRowsTPC, RatioCrossedRowsOverFindableClustersTPC, chi2TPC, isTPCrefit, isITSrefit, nSPD, nKinkDaughters, chi2ITS, isPropagatedToDca, DCAxy, DCAz, eta);
  ...
  hisPropagatedToDca[1]->Fill(isPropagatedToDca);
	hnCrossedRowsTPC[1]->Fill(nCrossedRowsTPC);
	hRatioCrossedRowsOverFindableClustersTPC[1]->Fill(RatioCrossedRowsOverFindableClustersTPC);


  return kTRUE;
}
//_____________________________________________________________________________
void AliAnalysisTaskNuclei::TrackSelectionMonitor(Float_t nCrossedRowsTPC, Float_t RatioCrossedRowsOverFindableClustersTPC, Double_t chi2TPC, Bool_t isTPCrefit, Bool_t isITSrefit, Int_t nSPD, Int_t nKinkDaughters, Double_t chi2ITS, Bool_t isPropagatedToDca, Double_t DCAxy, Double_t DCAz, Double_t eta) {

  hCheckTrackSel->Fill(0);

	// before the constraint was nTPCclusters>=70, as the ClusterCut == 0
  if(nCrossedRowsTPC>=70) {
	  hCheckTrackSel->Fill(1);

		if(RatioCrossedRowsOverFindableClustersTPC>=0.8) {
								...
								if(chi2ITS<=36) {
									...
									if(isPropagatedToDca) {
										...
										if(TMath::Abs(DCAxy)<0.5) {
											...
											if(TMath::Abs(DCAz)<1.0) {
												hCheckTrackSel->Fill(11);

												if(TMath::Abs(eta)<0.8) {
													hCheckTrackSel->Fill(12);
												}
											}
										}
									}
...
//_____________________________________________________________________________
void AliAnalysisTaskNuclei::PtCorr(Double_t &pt, Double_t nsigmaTPC[9]) {
...
//_____________________________________________________________________________
void AliAnalysisTaskNuclei::GetNsigmaTPC(AliVTrack *track, Double_t nsigmaTPC[9]) {
...
//_____________________________________________________________________________
Bool_t AliAnalysisTaskNuclei::IsTOFmatching(AliVTrack *track) {
...
//_____________________________________________________________________________
void AliAnalysisTaskNuclei::GetExpTOFtime(AliVTrack *track, Double_t p, Double_t exptimes[9]) {
...
//_____________________________________________________________________________
Double_t AliAnalysisTaskNuclei::GetBeta(AliVTrack *track, Double_t pt, Double_t nsigmaTOF[9]) {
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskNuclei::GetPdgCode(AliVParticle *mcpart) {
...
//_____________________________________________________________________________
Int_t AliAnalysisTaskNuclei::GetMCSpec(AliVParticle *mcpart) {
...
//_____________________________________________________________________________
Double_t AliAnalysisTaskNuclei::GetM2(Double_t p, Double_t beta) {
...
//_____________________________________________________________________________
void AliAnalysisTaskNuclei::ForPtCorr(Double_t pt, Double_t t_pt, Int_t kSpec) {
...
//_____________________________________________________________________________
Bool_t AliAnalysisTaskNuclei::IsTOFgoodmatching(AliVTrack *track, Int_t label, Double_t nsigmaTOF[9], Int_t kSpec, Double_t t_pt, Bool_t isPrimary, Bool_t isSecMat, Bool_t isSecWeak) {
